...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% Hopper Comparison Program 
% % Functions for each control input and one general neumerical Integrator
% clear 
% clc 
% clf
% close all
...
% ks      =   0.224; %N/m
% b       =   0;    %N/ms
% accel_F        = 10.12;     %acceleration of vehicle from compressor 
% 
% x_0 = [ .31 0];
% 
% % desired_Height = 0.4165; %m
% desired_Height = 0.5; %m
% desiredVel     = 0;
% maxErr         = .1; % After reching height 
% 
% % External Acceleration on body
...
% for k = 1:length(t)-1
%     %Get control input (ON or OFF) for each system from functions
%     u_Hys(k)                              = hysterisis_Hopper(X_Hys(k,:),desired_Height,maxErr);
%                                                                        %(kp,kd,ki,duty_Cycle,period,X_k,d,desiredVel,int_Err,previous_Error,dt,t_k,Ts,u_ON)
%     [u_Disc_PID(k), int_Err,duty_Cycle,Ts,previous_Error,u_Last]= PIDPWM_Hopper(kp,kd,ki,duty_Cycle,period,X_PID(k,:),desired_Height,desiredVel,int_Err,previous_Error,dt,t(k),Ts,accel_F,u_Last);
% 
%     [u_Cont_PID(k),int_Err_cont,previous_Error_Cont]          = PID_Cont_Hopper(kp,kd,ki,X_PID_Cont(k,:),desired_Height,desiredVel,int_Err_cont,previous_Error_Cont,dt);
%     [u_MPH(k),tau,T]                      = hysterisisXMaxPrincipal_Hopper(X_MPH(k,:),t(k),desired_Height,maxErr,alpha,beta,tau,T);
% 
%     u_Hys(k)      = u_Hys(k)*accel_F + g;
%     u_Disc_PID(k) = u_Disc_PID(k)*accel_F + g;
%     u_MPH(k)      = u_MPH(k)*accel_F + g;
% 
%     X_Hys(k+1,:)      = (I+A*dt)*X_Hys(k,:)'      + B*dt*u_Hys(k);
...
% 
% % getPosGraph(X_Hys,t,Tend)
% getGraphs(X_Hys,u_Hys,t,Tend,"Hysterisis")
% % getGraphs(X_PID_Cont,u_Cont_PID,t,Tend,"PID Cont")
% % getGraphs(X_PID,u_Disc_PID,t,Tend,"PID")
% % getGraphs(X_MPH,u_MPH,t,Tend,"MPH")
% 
% % compare2(X_Hys,X_PID,u_Hys,u_Disc_PID,t,Tend,"Hys","PID",desired_Height)
% name_All = ["Hys","Disc","MPH"];
% % getOverlayGraph([X_Hys,X_PID,X_MPH],[u_Hys,u_Disc_PID,u_MPH],t,Tend,name_All,desired_Height)
...
% 
% 
% function [u,tau,T] = hysterisisXMaxPrincipal_Hopper(X_k,t_k,d,maxErr,alpha,beta,tau,T)
%     % Time Calcs - Max Principal equations
% 
%     %Currrent Issue, Undershooting for all tau T besides first
%     b = X_k(1,2); % b = initial Velocity 
...
%     elseif (tau < t_k) && (t_k < T)
%         u = 0;
%     elseif (b < 0) && (c-maxErr < d)
%         tau = t_k + -(alpha*b + b*beta - alpha*(((beta + alpha)*(- 2*beta*c + b^2 + 2*d*beta))/alpha)^(1/2))/(beta^2 + alpha*beta)
%         T   = t_k +  b/alpha - ((beta + alpha)*(alpha*b + b*beta - alpha*(((beta + alpha)*(- 2*beta*c + b^2 + 2*d*beta))/alpha)^(1/2)))/(alpha*(beta^2 + alpha*beta))
%         u = 1;
%     else
%         u = 0;
%     end
...
% end
% 
% function getGraphs(X,control,t,Tend,name)
% 
%     arrayPosition = X(1:length(X),1);
%     arrayVelocity = X(1:length(X),2);
% 
...
%     title('Position Vs Time')
%     xlim([0 Tend])
%     ylim([min(arrayPosition), max(arrayPosition)])
%     xlabel('time')
%     ylabel("Position (m)")
% 
%     %Velocity Vs Time
...
% end
% % Stats
% function [stableHertz,timeOpenPerSec,averagePosStable] = getStats(u,dt,X)
% 
%     stableHertz = sum(u(end- 1/dt:end));
%     timeOpenPerSec = dt*stableHertz;
% 
